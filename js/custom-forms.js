...
//     const name = item.querySelector('.name div');
...
let nameItem = Array.from(document.querySelectorAll('.item-row .name div'));
nameItem.forEach(function(item, i, arr) {
    item.addEventListener('click', () => {
        edit(item.closest('.item-tb'));
    }); 
});
...
//     let modal = document.getElementById('modalImg');
//     const target = e.target;
//     const condition = target == modal || modal.contains(target);
//     if (!condition) {
//         toggleMenu();
//     }
// });